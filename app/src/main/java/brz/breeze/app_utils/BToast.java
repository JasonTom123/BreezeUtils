...
public class BToast extends Toast {
	...
	public BToast(Context context){
		...
		try{
			eng = Typeface.createFromAsset(am,"font/eng_1.ttf");
			chi = Typeface.createFromAsset(am,"font/chi_1.ttf");
		}catch(Exception e){Toast.makeText(context,e.toString(),0).show();}
	...
	private GradientDrawable getRound(){
		...
		g.setCornerRadius(90);
		return g;
	}
	
	public BToast setTypeface(int mode){
		if(mode == 0){
			textView.setTypeface(chi);
		}else{
			textView.setTypeface(eng);
		}
		return this;
	}
	...
	public static BToast toast(Context context,String content,int duration){
		...
		mToast.setTypeface(FONT_CHINESE);
		return mToast;
	}
}
